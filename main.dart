...
import 'package:svk/pages/catalog/catalog.dart';
import 'package:svk/pages/admin/сategoryad.dart';
import 'package:svk/pages/admin/products.dart';
import 'package:svk/pages/admin/tableproducts.dart';
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      routes: {
        ...
        '/profile': (context) => ProfilePage(token: ModalRoute.of(context)!.settings.arguments as Token),
        '/admin/dashboard': (context) => AdminDashboard(userRole: 4),
        '/users': (context) => UserListPage(),
        '/moderators': (context) => ModeratorList(),
        '/catalog': (context) => CatalogPage(),
        '/categories': (context) => AdminCategoryPage(),
        '/edit_product': (context) => ProductTablePage(),
      },
...
class MainPage extends StatelessWidget {
  ...
  Widget _buildDesktopHeader(BuildContext context) {
    ...
    children: [
    _buildNavItem('Главная', context),
    SizedBox(width: 30),
    _buildNavItem('Каталог', context),
    SizedBox(width: 30),
    _buildNavItem('Информация', context),
    ],
    ),
  ...
  Widget _buildNavItem(String title, BuildContext context) {
    return GestureDetector(
      onTap: () {
        if (title == 'Каталог') {
          Navigator.pushNamed(context, '/catalog'); // Переход на страницу каталога
        } else {
